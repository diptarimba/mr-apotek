...
@section('content')
    <x-util.card title="{{ $data['title'] }}">
        <x-form.base url="{{ $data['url'] }}" method="POST">
            ...
            <x-form.input :disable="is_null($notApproved) ? null : ($notApproved ? null : true)" name="quantity_received" label="Quantity" type="number"
                placeholder="input quantity" value="{{ $product->quantity_received ?? '' }}" />
            ...
            <x-form.input :disable="is_null($notApproved) ? null : ($notApproved ? null : true)" name="buy_notes" label="Catatan" placeholder="input buy notes"
                value="{{ $product->quantity_received ?? '' }}" />
            <x-form.input :disable="is_null($notApproved) ? null : ($notApproved ? null : true)" name="expired_at" label="Tanggal Expired" type="date"
                placeholder="input contact name" value="{{ $product->expired_at ?? '' }}" />
